# agentic-host

The methodology's meta and management home, and itself an agentic project that
develops the `host-*` [components](STRUCTURE.md#components).

## Adopters: do not treat this repo's top-level contents as methodology

This repository is **both** the methodology's meta home **and** a project instance
with its own five rooms. Its top-level `call/`, `plan/`, `MEMORY.md`, and the
project-specific parts of its `AGENTS.md` are **its own instance rooms and bind no
adopter**. The normative methodology you inherit lives in
[`host-template/`](host-template), the versioned source you copy-at-version. A bare
`call/NNNN` reference resolves against *your* template's numbering, not this repo's.

This is the outward dual of the "Template manual exemption" in this repo's
`AGENTS.md` (which tells an agent working *here* to ignore the nested template as
governance). Together they fix the boundary in both directions.

See `STRUCTURE.md` for the room map.
