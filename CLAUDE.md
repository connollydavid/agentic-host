...
- Software (the *Where* room): `host-lint/`, the canonical worktree of the `host-lint` bare store (`host-lint.git/`), embedded as a bare-store-with-worktrees and pinned in `.host-software` (both gitignored, materialized locally). A Rust CLI (`host-lint`) that detects phase-synonym agentic tells in commit messages, markdown headers, and code comments. VOCABULARY.md in `host-lint/` is the source of truth for detection rules.
...
- Fresh-clone setup: the worktree is absent until materialized, so a fresh clone first runs `host-lifecycle software --materialize .`, then `git submodule update --init` (the tool submodules `tools/{allium,specula,host-lifecycle}` and `host-template`) and `./link-skills.sh` to generate the tool skill symlinks under `.claude/skills/` (gitignored), and recreates the software skill symlink `ln -s ../../host-lint .claude/skills/host-lint` (also gitignored, never tracked, or it would dangle in CI; worktree-absence coherence). Then it builds host-lint (`cargo build --release` in `host-lint/`) and installs the commit hooks with `host-lifecycle software --install-hooks .` (copies the `hooks` dispatch script in as `pre-commit`/`commit-msg` plus the built binary; gates on the worktree being at its pin, so a local build is accepted). `host-lifecycle software --check .` must be clean (worktree at its pin, no tracked symlink into a worktree). The verification-lane tools are wired per the spine MUST (allium + specula); their `.allium`/`.tla` specs live in the software repos (see `plan/0012`).
