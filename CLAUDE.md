...
- Fresh-clone setup: the worktrees are absent until materialized, and `host-lifecycle` is itself one of the components it would materialize, so a fresh clone seeds it first: `cargo install --git https://github.com/connollydavid/host-lifecycle --rev <the host-lifecycle pin recorded in .host-software>` (the materializer cannot be served from what it materializes). Then `git submodule update --init` (the submodules are `tools/{allium,specula}` and `host-template`), `host-lifecycle software --materialize .` to clone every `.host-software` component into `software/<name>/main/`, and `./link-skills.sh` to generate the skill symlinks under `.claude/skills/` from both the tool submodules and the materialized worktrees (gitignored, never tracked, or they dangle in CI; worktree-absence coherence, call/0005). Build host-lint at its recorded `x86_64-unknown-linux-musl` target so the binary lands at the artifact path the hook install reads (`rustup target add x86_64-unknown-linux-musl`, then `cargo build --release --target x86_64-unknown-linux-musl` in `software/host-lint/main/`; the digest-pinned recipe in `.host-software` reproduces the byte-identical canonical binary). Install the commit hooks with `host-lifecycle software --install-hooks .` (copies the `hooks` dispatch script in as `pre-commit`/`commit-msg` plus the binary at the recorded artifact path; gates on the worktree being at its pin). The canonical static-musl binary runs on the dev host, so `--install-hooks` installs the canonical binary itself (plan/0032 retired the plan/0028 local-glibc-build workaround that installed a binary differing from the canonical hash). Because agentic-host declares deeper verification rungs (`kani:`/`apalache:`/`tlaps:` dispositions), install the shared re-deriver host-prove on PATH so `software --check`'s runnability gate passes: `cargo install --path software/host-prove/main --root ~/.local` (its pin is recorded in `.host-software`; the gate HAZARDs a declared rung whose re-deriver does not run, plan/0048). `host-lifecycle software --check .` must be clean (every worktree at its pin, no tracked symlink into a worktree, every component's embed and release receipt present). The verification-lane tools are wired per the spine MUST (allium + specula); the `.allium`/`.tla` specs live in the software repos (see `plan/0012`).
