...
- Fresh-clone setup: the worktree is absent until materialized, so a fresh clone first runs `host-lifecycle software --materialize .`, then recreates the local skill symlink `ln -s ../../host-lint .claude/skills/host-lint` (gitignored — never tracked, or it would dangle in CI; worktree-absence coherence), then builds host-lint (`cargo build --release` in `host-lint/`) and installs the commit hooks with `host-lifecycle software --install-hooks .` (copies the `hooks` dispatch script in as `pre-commit`/`commit-msg` plus the built binary; gates on the worktree being at its pin, so a local build is accepted). `host-lifecycle software --check .` must be clean (worktree at its pin, no tracked symlink into a worktree).
