...
### Goals
- [x] Fix pre-commit hook: dispatch on `$(basename "$0")` because git passes no hook name in `$1`; commit-msg receives the message file path (verified in throwaway repo)
