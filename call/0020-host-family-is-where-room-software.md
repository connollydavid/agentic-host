...
release/version); giving each tool its own agentic project is **not** required. agentic-host stays the
...
- host-lifecycle, host-prove, host-grammar stop being `tools/` submodule / git-dep references and
  become `.host-software` Where components (see `plan/0028`); `tools/` then holds only allium + specula.
