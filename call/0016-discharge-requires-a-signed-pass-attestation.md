...
`obligation_gaps` calling `src.contains(name)`: it checked the proof was **present by name**, never
that it **passed**. So a stub, a failing check, or a vacuous `THEOREM X == TRUE` satisfied
