...
a **bare clone + worktrees**; but that is not a gitlink, so adopting
...
- **Recipe, not trees.** The bare store and all worktrees are local artifacts
  ...
  submodules per worktree, then add the parallel worktrees.
...
### Migration (an existing submodule to a bare store)

A project already embedding its software as a gitlink converts mechanically,
**preserving the software exactly**, with no software commit created, rewritten, or
moved:
...
5. **Path continuity is the win.** The canonical worktree keeps the path `<name>/`.
   Only the embedding mechanism changed, so every reference that pointed at
   `<name>/…` (build commands, routing tables,
   hook paths) still resolves. Audit
...
- Neutral: audit shifts off native `git submodule status` onto a `host-lifecycle`
  check against the recorded SHA; a fresh host clone no longer auto-fetches the
  software; the setup step (recipe, then materialize) replaces `submodule update
  --init` and must be documented/scripted.
- Bad / accepted: worktrees share git objects while keeping separate environments;
  each carries
