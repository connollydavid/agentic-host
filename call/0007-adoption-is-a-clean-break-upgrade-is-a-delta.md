...
- **Adoption** (first-time: case (a)/(b), or a first-stamping case (c)): a
  *transformation* of what the repo is, so a **clean break to a pristine live state.**
- **Upgrade** (a stamped case (c) bumping template revision X to Y): a *delta*, so
...
- Cost: "adoption = clean break" raises the one-time spend and the tier-2
  ...
  record rewrite costs more than it is worth.

### Per-target consequences (correcting the `call/0005` audit)

- **Agentic-MCP-Win32s** (adoption): clean break. Live PHASE docs to content homes
...
- **pgs-release** (adoption): **Staged-Shallow for the host migration, by cost,
  ...
  PHASE docs into content homes (tier 1); acknowledge host git history (tier 3); keep
