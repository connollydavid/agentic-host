...
> **Status: done (2026-06-14).** Executed as template `0e5dd7f` to `59315bb`
...
to Unlicense. This is the coupled CLAUDE.md-rewrite leading to the Unlicense-flip.
...
2. **Rewrite `CLAUDE.md` as the canonical methodology** in our own words (this is
...
3. **Reframe `README.md`** as "the host template": what it is, how to
   instantiate (clone, then `git submodule update --init`, then replace `cast/` examples, then
   add the software submodule), Provenance, Unlicense.
4. **Flip `LICENSE` from MIT to Unlicense** (the content is then ours; Provenance carries
...
- **D1: CLAUDE.md scope, full cut, tightly framed.** Write the full methodology
  ...
  criterion.
- **D2: Host and template sole-source, defer liberally.** The host builds the
...
- **Done.** Bumped `tools/host-lint` pointers (template + host) `a9a7cd3` to
...
- **Done.** Refreshed the host's installed git hooks: renamed the binary
  `.git/hooks/no-phase` to `host-lint` and reinstalled the canonical `pre-commit`
...
## D2 (deferred): host and template sole-source
