...
| [plan/0014-mandate-and-wire-the-lanes](plan/0014-mandate-and-wire-the-lanes/README.md) | in progress (tighten the spine to a conditional MUST — a present `.allium`/`.tla` requires its tool, skills, and CI lane, full allium lifecycle incl. `plan` obligations; UPGRADING entry for all upgraders; then test the migration by fully aligning agentic-host — wire `tools/{allium,specula,host-lifecycle}` + generated skills) |
