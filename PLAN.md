...
| [plan/0012-specs-live-with-software](plan/0012-specs-live-with-software/README.md) | in progress (dogfood real `allium-cli 3.4.2` with the `analyse` advanced gate, wired in each software repo's CI; relocate `ParallelScan.tla` from the host into `host-grammar`; spine fix — specs co-locate with software, the "What" room moves out of the host) |
