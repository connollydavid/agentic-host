...
- **No surface outside host-lint may gate character content (audited 2026-09-05, operator ruling: the Latin alphabet is fully permitted, the section sign and arrows were the decoration family).** The audit covered the git attributes (line-ending normalization only), every workflow, the shell scripts and the entrance installer (no charset checks), and every `is_ascii` use in host-lifecycle (all token parsers recognizing ASCII-shaped identifiers: register and issue numbers, slug shapes, review-finding codes; the reference walk's boundary check is Unicode-aware `is_alphanumeric`). Unicode language content answers to host-lint's decoration tropes alone, and the implementation of this plan must not introduce a second gate: the census counts, the strict tier escalates warnings, and nothing else reads bytes.
