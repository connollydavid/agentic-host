...
>
> **Status: SHIPPED** in host-lint **v0.8.0** (`e652ffc`), with occurrence-mapping + fix-hints + advisory
...
> host-* Where component, and it surfaced + fixed two release-tool bugs (yielding host-lifecycle **v0.19.1**).
...
- **Mislocalization (the headline defect).** `call/0019` has **ten** em-dashes (lines 12, 15, 23, 24,
  ...
  ten records collapse onto `line 12`. A weak agent sees ten identical records on `line 12` and cannot tell
...
- The **density-score concern is moot,** since `tell_score` counts correctly because the engine emits the
...
- **E1 + E2: locate each occurrence with a column.** Replace `locate_line`'s first-occurrence search
  ...
  positions of the excerpt in the source and assign tell *k* to occurrence *k*, yielding `file:line:col`.
  Ten em-dashes yield ten records at their real `line:col`; one em-dash yields one record. Output becomes
  `path:line:col: …`.
- **E3: a mechanical fix-hint** keyed on the trope `id` (e.g. `decoration` maps to "drop it, or replace with
  a comma/period"; `arrow` maps to "use a word"), in the text line and as a `fix` field in `--json`. Only
...
- **E4: advisory tiers.** A tell whose excerpt does **not** appear literally in the source (the
  ...
  source yields locatable, not found yields advisory, so no hardcoded id list to maintain.)

## Verification

- **Golden tests** (host-lint): the `call/0019` regression, ten `decoration` records at the ten real
  em-dash lines (not ten at line 12), each with a column; one em-dash yields one record; a mechanical trope
...
- **4B acceptance: PASSED (atomic).** Qwen-3.5-4B (`unsloth/Qwen3.5-4B-MTP-GGUF`) applied the located +
  fix-hinted output correctly **one fix at a time**: a mechanical decoration fix (replacing em-dashes with
  commas) and a no-fix reword (`delve` to `look at`), and it **correctly ignored the advisory `note`**. It
...
host-lint-only: **host-lint** (E1 to E4 + golden tests, on top of the already-built D1), then `cargo test` +
clippy + integration green, then 4B acceptance, then bump version, then re-pin `.host-software`, then `--verify-build`, then
CI. No upstream repo.
...
- Occurrence-mapping matches by excerpt string. It is exact when tell-count == occurrence-count (the
  ...
  (curly-to-straight quotes, collapsed whitespace), that tell would not be found and would fall to the
