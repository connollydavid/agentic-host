...
- Unit test `refuse_adopting_software_in_place`: empty dir yields proceed; `Cargo.toml`
  yields refuse (names the manifest); `.host` stamp or `.host-software` yields proceed.
- CLI smoke: empty yields `a`; software dir yields refusal + exit 3; host root yields `c`.
