...
The front-door check (plan/0040, host-lifecycle v0.27.0) holds the one published front
door of agentic-host to the spine. The same problem recurs wherever a project keeps a
...
the `.host-software` member marked `front-door = true` and checks its README against the
...
- **Declared out of the prose.** A project declares which documents are entrances, and
  ...
  `.host-software`, generalizing the `front-door = true` flag), never a marker inside the
...
- **One concept set or many.** A front door restates every concept; a skill may restate
...
- **The rename blast radius.** `front-door` to `entrance` across the subcommand, the
  `front-door = true` flag, the tests, and the agentic-host front door (the first
  instance migrates to the general form).
...
Generalize the front-door check to a declared, opt-in entrance check, rename `front-door`
to `entrance`, and keep the routing rule (generate format blocks, coverage prose). The
unit tests pass.
...
Migrate the agentic-host front door to the general entrance form (the first instance),
...
by Qwen-3.5-4B data; the standalone sibling of reconcile). Awaiting adversarial review of
the open decisions before building. Supersedes the agentic-host-local scope of plan/0040
