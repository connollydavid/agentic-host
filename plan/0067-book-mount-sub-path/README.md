...
## The spine side (shipped in the template roll)
...
`requires` on the host-lifecycle release above. This half shipped as host-template `962630c` (the
mount-aware Site workflow, the `book --print-mount` publish recheck, and the `book-mount` doctrine, with
the UPGRADING ledger entry keyed to it), and agentic-host adopted it through the ledger; its own
doc-site workflow is now mount-aware, though it still publishes at the root.
