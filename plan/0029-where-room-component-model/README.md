...
- **declares `artifact`** → `--verify-build` rebuilds from the pin and must reproduce the hash;
  `release` builds, re-derives, re-pins, tags, receipts. (A binary, a JS `dist/`, a wheel, a `.so` —
  all just "declares an artifact".)
- **no `artifact`** → `--verify-build` no-ops; `release` is **tag-only**. *This path already exists in
...
- **no `build`** → `--check` validates the pin only (a text/docs component; `host` proves this works).
...
- **Uniform for every adopter, including single-component ones** (operator decision: reduce churn over a
...
- **Omit `--item` → all components, canonical worktrees** (today's behavior; the bare `[<dir>]`
  positional stays exactly where it is). The specifier is a **flag** because the existing parser already
  takes the first positional as `<dir>` — a second positional `<item>` would be genuinely ambiguous
  (`software --check host-lint`: item or dir?).
- `--item <name>` → that component's canonical worktree (the targeted bootstrap/check/build).
- `--item <name>@<branch>` → a specific branch worktree (`<branch>` keeps its slashes).
- **`@<branch>` is never propagated into `--component` for receipts** — the receipt gate keys
...
The spine states "Reference, don't vendor — each tool is a git submodule … do not copy its code into
...
- An **adopter** *references* the host-* tools — pinned submodules, "reference, don't vendor".
...
The five-step apply sequence, stated explicitly: commit the spine change in `host-template` → add the
`[upgrade "<rev>"]` entry with the state-check `verify=` → bump agentic-host's `.host` to that revision
→ `host-lifecycle upgrade --record <id>` (runs the `verify=`) → re-materialize. The re-materialize is
itself an ordered tool-run sequence — **never by hand**: a clean-worktree precondition (refuse a *dirty*
parallel worktree, so uncommitted work is never lost) → update `.gitignore` to `/software/` → materialize
(pruning stale admin first) → an explicit **teardown** of the old `<name>/`, `<name>.git/`,
`<name>.<line>/` dirs (a tool step) → drop the old ignore triplets. Note the field-change migration
