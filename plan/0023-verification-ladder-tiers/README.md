...
Resolves connollydavid/host#3 (no solver tier, so parametric/unbounded invariants could
...
- `apalache-symbolic` drives Apalache (TLA+ to SMT/Z3): parametric/unbounded `.tla` invariants.
- `tlaps-proof` drives TLAPS (`tlapm`): unbounded proof. Authoring needs a strong model; the
  skill scopes a weak model to running/maintaining existing proofs.
- `kani-conformance` drives Kani: Rust code↔spec conformance.
...
- `spec_lane_problems` raises a **conditional** HAZARD, firing only when a `.obligations`
...
- **Kani in host-lint** (`a7ae5e9`): three `#[cfg(kani)]` proofs, `is_review_code`
  (#digit is a code; two letters are not, discharging `rule-{success,failure}.Detect
...
failing proof even when the user pushed for it*. Where it slipped was free-form proof
...
a fixed verdict vocabulary) makes the rung reliable regardless of the serving layer.
