...
- **Skill-wiring is a real mechanism change** that touches `link-skills.sh` and the fresh-clone setup;
...
- **host-grammar as a library Where component** has no deployable artifact, so `release` is tag-only;
