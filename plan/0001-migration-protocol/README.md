...
- **`host-lifecycle` migrate verbs** (`adopt` / `version` / `classify`, with
  ...
  write and read the `.agentic-host` stamp, report the migration case.
- **`template-agentic-host/MIGRATION.md`**: the protocol payload, on two
...
- **This host, migrated (case c, Shallow):** stamped at the template revision in
  `.agentic-host`; the agentic-host model and copy-at-version sourcing recorded
