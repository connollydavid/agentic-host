...
local fallback ran). Ten independent finder angles ran in parallel: five correctness groups across the
crates, plus the overlay lens laws, a cross-cutting determinism audit, the CLI and core dispatch, the
conformance fixtures and removed-behaviour audit, the specification and CI coverage, and a reuse and
altitude pass. Targeted verification followed (several findings were confirmed by executing the code at
the reviewed revision), then one gap sweep. Findings were deduped and ranked.
...
10. **Both out-of-process helpers stage input at a predictable shared temp path.**
    ...
    symlink attack); two concurrent calls on the same bytes collide, with one's truncate or
    `remove_file` racing the other's helper read. That race is non-deterministic and surfaces as an
    intermittent "helper failed". An `O_EXCL` temp file with a random suffix is the fix.
...
- **The HTML `view` reports the wrong source-map origin.** `crates/html/src/lib.rs:99`. `view`
  ...
  origin of the returned text, so it breaks the bidirectional source map. (The `data` `view` is
  correct because its text is the source bytes; the HTML conversion makes the offsets incomparable.)
...
- **The `view` of most metadata readers ignores its selector.** `crates/rtf/src/lib.rs:50`, and
  ...
  `pdf`. A windowed or token-budgeted request silently returns the whole document and blows any token
...
- **The golden-test scaffolding is copy-pasted across roughly twenty-three `conformance.rs` files**
  (`crates/*/tests/conformance.rs`) and differs only by the normaliser type name. A shared
...
- **The conformance fixtures are sound.** Every normaliser crate ships a `tests/conformance.rs` on one
  ...
  no self-comparison, no early return. The OCR test golden encodes a stub helper's fixed output
  (finding 9), which is correctly scoped to this crate rather than the engine that lives out of
  process.
...
- **Workspace coverage is consistent.** Every crate directory is in the workspace
...
panicking, hanging, or silently zeroing. That is the call/0031 contract the component asserts but does
not yet keep. The determinism discipline and the conformance fixtures are sound foundations to build
that on.
