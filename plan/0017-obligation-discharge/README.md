...
explicit disposition, mechanically checked, per component, via the method's own
...
- `host-lint.obligations`, all **44** obligations: spec-integrity to `structural`;
  the five detection rules and the verdict-lifecycle transitions to named property
  tests in `property_tests.rs` (e.g. `flag_wins_over_warn_on_the_same_line`);
  `StartScan` waived.
- `host-grammar.obligations`, all **39**: structural-equation rules to named PBTs
