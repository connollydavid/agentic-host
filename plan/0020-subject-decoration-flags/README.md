...
- CLI: em-dash subject → flag (exit 1); em-dash in body only → warn (exit 3);
  clean → exit 0.
- `allium check`/`analyse` (3.4.2) clean; `host-lifecycle obligations` → all 47
  dispositioned; `software --check` clean.
- Reproducible: clean-room `rust:1.95.0` container build of `1d0010c` →
