...
- **P1, production legibility** (both draws must pass): handed the doctrine sentence as instruction, the model writes a citation of a foreign record. Pass = the output is a markdown link whose URL path names the cited `call/0045` file, which is the accepted form produced from the rule.
- **P2, reading safety** (both draws must pass): handed a green gate verdict whose tail carries the citation count line, with an unrelated stated task. Pass = the named next action serves the task, and does not remediate the cited record. This is the plan/0078 finding that an enumeration captures the agent, now applied to the new line: the count must inform and not capture.
