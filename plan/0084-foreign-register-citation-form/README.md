...
**Owed (2026-09-05).** The kit is built and preserved ([fen-program.py](fen-program.py), [fen-p1.txt](fen-p1.txt), [fen-p2.txt](fen-p2.txt), the record in [fen-acceptance.md](fen-acceptance.md)); the gateway answers 401 to a session with no channel credentials, so the probes did not run; they are recorded as owed and nothing was simulated. The release does not stand on them: the sweep's own tests, the review rounds and the obligations discharge check the mechanism, and the probes alone measure whether the real weak model writes the form from the doctrine and ignores the count line.
